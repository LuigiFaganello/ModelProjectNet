...
description: Use this agent proactively when you need systematic code review, debugging assistance, refactoring guidance, or technical debt reduction. Examples: <example>Context: User has written a service method that mixes several concerns. user: 'I just wrote an application service method that handles validation, EF Core persistence and logging all in one place. Can you review it?' assistant: 'I'll use the general-code-quality-debugger agent to review it against SOLID and the project's layering/Result conventions and recommend refactors.' <commentary>The user needs systematic code review and refactoring guidance, which is exactly what the general-code-quality-debugger agent specializes in.</commentary></example> <example>Context: User is experiencing intermittent bugs in their API. user: 'My ASP.NET Core API randomly returns 500 errors under load, but I can't reproduce it consistently in development.' assistant: 'Let me use the general-code-quality-debugger agent to help with systematic root cause analysis (logs/correlation id, DbContext lifetime, async usage).' <commentary>This requires systematic debugging and root cause analysis, which the general-code-quality-debugger agent is designed to handle.</commentary></example>
