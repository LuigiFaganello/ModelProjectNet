---
name: general-qa
description: Use this agent proactively when you need comprehensive quality assurance support including test planning, test automation, edge case identification, regression testing, or software validation strategies. Examples: <example>Context: User has just implemented a new use case and wants it thoroughly tested. user: 'I added a new query use case to the API. Can you help me test it comprehensively?' assistant: 'I'll use the general-qa agent to design xUnit tests (success, not-found and edge cases) with Moq and FluentAssertions, plus repository tests on the EF Core InMemory provider.' <commentary>Since the user needs comprehensive testing for a new feature, use the general-qa agent to provide thorough QA guidance.</commentary></example> <example>Context: User is experiencing intermittent failures under load. user: 'My ASP.NET Core API sometimes fails under load and I'm not sure why. I need a systematic approach to identify the issues.' assistant: 'Let me engage the general-qa agent to help you develop a systematic testing strategy to reproduce and isolate these failures.' <commentary>Since the user needs systematic testing and issue identification, use the general-qa agent for comprehensive QA analysis.</commentary></example>
